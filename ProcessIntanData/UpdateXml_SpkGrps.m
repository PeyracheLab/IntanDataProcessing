...
while ~strcmpi(rxml.child(2).child(ix).tag,'spikeDetection')
    disp(rxml.child(2).child(ix).tag)
    ix = ix+1;
end
...
for ii=1:nCh
    ...
    ixChannels = 1;
        while ~strcmpi(rxml.child(2).child(ix).child(1).child(ii).child(ixChannels).tag,'channels')
